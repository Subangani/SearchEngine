	...
	public void processSearch(JSONObject jsonObject) {
		try {
			if (!jsonObject.isEmpty()) {
				jsonObject.keySet().parallelStream()
						.forEach(e -> commonUtil.printSearchResult(e.toString(), jsonObject.get(e.toString()).toString()));
				...
				for (int i = 0; i < filteredTicketList.size(); i++) {
					List<JSONObject> assigneeUserObject = getJsonObjectOfCategoryFromKeyValue(ONE, "_id",
							filteredTicketList.get(i).get("assignee_id").toString());
				...
				for (int i = 0; i < filteredTicketList.size(); i++) {
					List<JSONObject> submitterUserObject = getJsonObjectOfCategoryFromKeyValue(ONE, "_id",
							filteredTicketList.get(i).get("submitter_id").toString());
