...
public class OrganizationSearchService extends SearchService {
	...
	public void processSearch(JSONObject organizationObject) {
		try {
			if (!organizationObject.isEmpty()) {
				organizationObject.keySet().parallelStream().forEach(
						e -> commonUtil.printSearchResult(e.toString(), organizationObject.get(e.toString()).toString()));
