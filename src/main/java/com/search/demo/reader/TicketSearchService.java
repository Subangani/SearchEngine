...
public class TicketSearchService extends SearchService {
	...
	public void processSearch(JSONObject ticketObject) {
		try {
			if (!ticketObject.isEmpty()) {
				ticketObject.keySet().stream()
						.forEach(e -> commonUtil.printSearchResult(e.toString(), ticketObject.get(e.toString()).toString()));
				...
				List<JSONObject> assigneeUserObject = getJsonObjectOfCategoryFromKeyValue(ONE, "_id",
						ticketObject.get("assignee_id").toString());
				...
				List<JSONObject> submitterUserObject = getJsonObjectOfCategoryFromKeyValue(ONE, "_id",
						ticketObject.get("submitter_id").toString());
